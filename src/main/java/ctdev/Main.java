...
public class Main {
    static final String DB_CONNECTION = "jdbc:mysql://localhost:3306/dbsasha?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC";
    ...
    private static void viewSalary(Scanner sc) throws SQLException {
        System.out.print("Enter client name: ");
        String name = sc.nextLine();
       //PreparedStatement ps = conn.prepareStatement("SELECT * FROM Clients where salary = '100'");
        PreparedStatement ps = conn.prepareStatement("SELECT name, salary from clients where name = ?");
    ...
    private static void viewClientWithSalary(Scanner sc) throws SQLException {
        System.out.print("Enter min salary: ");
        String salmin = sc.nextLine();
        int minimum_salary = Integer.parseInt(salmin);
        System.out.print("Enter max salary: ");
        String salmax = sc.nextLine();
        int max_salary = Integer.parseInt(salmax);
        //Random rnd = new Random();
        PreparedStatement ps = conn.prepareStatement("SELECT name, age, salary from clients where salary BETWEEN ? and ?");

        try {
            // table of data representing a database result set,

            // ps.setString(1, name);
            ps.setInt(1, minimum_salary);
